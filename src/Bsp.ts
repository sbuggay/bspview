...
export class Bsp {
    ...
    constructor(buffer: ArrayBuffer) {
        ...
        this.edges = this.extractLump(buffer, lumps["EDGES"], ["Uint16", "Uint16"]);
        this.planes = this.extractLump(buffer, lumps["PLANES"], ["Float32", "Float32", "Float32", "Float32", "Uint32"]).map(data => {
            return {
                x: data[0],
                y: data[1],
                z: data[2],
        ...
        this.surfEdges = this.extractLump(buffer, lumps["SURFEDGES"], ["Int32"]);

        // [TODO] This depends on BSP version (1b vs 3b)
        this.lighting = this.header.id === 30
            ? this.extractLump(buffer, lumps["LIGHTING"], ["Uint8", "Uint8", "Uint8"])
        ...
        this.vertices = this.extractLump(buffer, lumps["VERTICES"], ["Float32", "Float32", "Float32"]).map(vertex => {
            return new Vector3(vertex[0], vertex[1], vertex[2]);
        });

        this.nodes = this.extractLump(buffer, lumps["NODES"], ["Uint32", "Int16", "Int16", "Int16", "Int16", "Int16", "Int16", "Int16", "Int16", "Uint16", "Uint16"]).map(data => {
            return {
                plane: data[0],
                front: data[1],
                back: data[2],
        ...
        this.leaves = this.extractLump(buffer, lumps["LEAVES"], ["Int32", "Int32", "Int16", "Int16", "Int16", "Int16", "Int16", "Int16", "Uint16", "Uint16", "Uint8", "Uint8", "Uint8", "Uint8"]).map(data => {
            return {
                type: data[0],
                vislist: data[1],
        ...
        // Parse visplane
        this.visibility = this.extractLump(buffer, lumps["VISIBILITY"], ["Uint8"]);

        // Parse textures
        this.texInfo = this.extractLump(buffer, lumps["TEXINFO"], ["Float32", "Float32", "Float32", "Float32", "Float32", "Float32", "Float32", "Float32", "Uint32", "Uint32"]).map(data => {
            return {
        ...
        // Parse models
        this.models = this.extractLump(buffer, lumps["MODELS"], ["Float32", "Float32", "Float32", "Float32", "Float32", "Float32", "Float32", "Float32", "Float32", "Int32", "Int32", "Int32", "Int32", "Int32", "Int32", "Int32"]).map(data => {
            return {
        ...
        // Parse faces
        this.faces = this.extractLump(buffer, lumps["FACES"], ["Uint16", "Uint16", "Uint32", "Uint16", "Uint16", "Uint32", "Uint32"]).map(data => {
            return {
                plane: data[0],
                side: data[1],
                firstEdge: data[2],
